    ...
    <div class="w-full lg:w-8/12 h-full">
      <div class="card bg-base-100 shadow-xl lg:mt-0 w-full mt-5">
        <div class="card-body">
          ...
          <NuxtLink to="/adobe-illustrator/tutorials"><button class="btn btn-success uppercase text-white">All Adobe Illustrator tutorials</button></NuxtLink>
        </div>
      </div>
    </div>
  </div>
...
const route = useRoute()
  ...
  let {data: article} = await useAsyncData('article',() => $fetch('/api/articles/get-article', {method: 'GET', query: {url: '/adobe-illustrator/tutorials/' + route.params.articleUrl}})  )
